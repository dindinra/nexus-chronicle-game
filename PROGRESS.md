...
- [x] **Fase 6** — Frontend baru: React+TS+Vite — **IN PROGRESS (2026-07-08)**
  ...
  - [ ] 6.7 Porting logic game (engine + interaksi) — **IN PROGRESS: 6.7c-1..6.7c-5 SELESAI (2026-07-15); sisa 6.7d (efek kartu) & 6.7e (AI lawan)**
    ...
    - [x] 6.7c Sistem giliran (turn/phase state) — **6.7c-1 s.d. 6.7c-5 SELESAI & terverifikasi E2E (2026-07-15)** — lanjut 6.7d (efek kartu)
    ...
    - [x] **6.7c-4** Gate `playCard`/slot per phase + tipe kartu + cek energi (port `handClick`/`slotClick`) — **SELESAI & terverifikasi (2026-07-13)**. Port verbatim dari prototype: `slotClick`(1555), `execAttack`(1703), `resolveAttackInPlace`(1811), `effAtk`(1481), `checkWin`(1213), `spendPlayerEnergy`(1141), `handSelect`(port handClick 1510). State baru: `selHand`, `selTrap`, `atkSrc`, `atkSrcRef`, `result`. `FieldRow`/`PlayerHandFan` di-refactor (onClick di `.nc-card`). Type stubs: `useFn`/`frontOnceFn`/`backOnceFn`/`trapFn`/`gyFn`.
      ...
      - [x] **Commit:** `a3289f9` (amend checkpoint `f747815` → 1 commit bersih 6.7c-4).
      - [x] **6.7c-5** Giliran musuh dasar (`startEnemyTurn` + `aiMainPhase` + `aiAttackSequence`) — **SELESAI & terverifikasi E2E (2026-07-15)**.
        - `aiMainPhase` (5a, `7ad4f8e`): enemy summon+trap+heal; **fusion DI-SKIP** (demo state belum punya data fusion — scope 6.7c-5 = giliran musuh DASAR).
        - `aiAttackSequence` + `decideAiAttackTarget` (5b, `5ecbf3b`): enemy serang tiap front-row (row-aware via `effAtk`), 400ms pacing, front-row-first rule, nc11 any-target.
        - `startEnemyTurn` wiring (scaffold dari 6.7c-3): draw(`applyEnemyTurnStart`) → main(`applyAiMainPhase`) → battle → `aiAttackSequence` HANYA jika `!firstTurn` → `proceedToEnd` → `startPlayerTurn`. Turn-counter +2/enemy-turn (quirk prototype, faithful).
        - **Verifikasi E2E browser (2.5 ronde penuh):** enemy turn 1 (`firstTurn=true`) TIDAK menyerang (LP 80/80); enemy turn 2 (`firstTurn=false`) MENYERANG (Player 80→79); turn 1→3→5. Screenshot `screenshots/6.7c-5c-*.png`. `npm test` 12/12 pass.
